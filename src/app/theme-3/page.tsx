...
const VERB_ING = ['reading', 'writing', 'drawing', 'playing', 'studying', 'cooking', 'watching'];
const NOUNS = ['book', 'movie', 'song', 'game', 'recipe', 'story', 'article'];
const TIMES = ['morning', 'afternoon', 'evening', 'night', 'weekend', 'today', 'yesterday'];
...
const SkeletonBox = ({ className = '' }: { className?: string }) => (
  <div className={`bg-white/60 border border-neutral-200 animate-pulse rounded ${className}`} />
);
...
function generateRandomMessage(): Message {
  ...
  const verbIng = randomChoice(VERB_ING);
  const noun = randomChoice(NOUNS);
  const time = randomChoice(TIMES);

  // decide has/have based on person/number
  const aux = pron.word.toLowerCase() === 'he' || pron.word.toLowerCase() === 'she' ? 'has' : 'have';
  const sentence = `${pron.word} ${aux} been ${verbIng} a ${noun} since ${time}.`;
...
// very naive phonetic placeholders (could be improved with real mapping)
...
export default function Prototype() {
  ...
  const generateBatch = () => {
    const batch: Message[] = Array.from({ length: 10 }, () => generateRandomMessage());
    setMessages(batch);
  };
    ...
    <main className="flex w-screen h-screen bg-gradient-to-tr from-gray-100 to-neutral-200">
      ...
      >
        ...
        <button
          ...
          className="mt-6 w-full bg-violet-600 hover:bg-violet-700 text-white py-2 rounded-xl shadow"
        >
          Generate 10 Random
        </button>
      </aside>
            ...
            {effectiveLoading ? (
              ...
              messages.map((msg, idx) => (
                <div key={idx} className="space-y-4">
                  {/* Tense Detected card with metadata */}
                  <div className="bg-gradient-to-br from-pink-100 to-white border-l-4 border-pink-400 p-4 rounded-xl shadow-lg text-pink-800 text-base font-semibold">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      ...
                      {msg.aspect && (
                        <div className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800 ring-1 ring-inset ring-purple-200">
                          {msg.aspect.replace(/\b\w/g, (c) => c.toUpperCase())} Aspect
                        </div>
                      )}
                      {msg.voice && (
                        <div className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 ring-1 ring-inset ring-blue-200">
                          {msg.voice === 'active' ? 'Active Voice' : 'Passive Voice'}
                        </div>
                      )}
                      {msg.timeReference && (
                        <div className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-teal-100 text-teal-800 ring-1 ring-inset ring-teal-200">
                          {msg.timeReference === 'ongoing'
                            ? 'Ongoing'
                            : msg.timeReference === 'now-relative'
                            ? 'Now-related'
                            : msg.timeReference === 'past-relative'
                            ? 'Past-related'
                            : 'Future-related'}
                        </div>
                      )}
