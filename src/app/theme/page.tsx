...
const DEFAULT_MESSAGES: Message[] = [
  {
    ...
    polarity: 'positive',
    questionType: 'none',
    recommendation: 'Good to continue practicing this tense.',
  },
];
...
const badgeBase =
  'inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ring-1 ring-inset mr-2 mb-1';

const classificationBadge = (label: string, variant: string) => {
  let bg = 'bg-gray-100 text-gray-800 ring-gray-200';
  if (variant === 'person') bg = 'bg-indigo-100 text-indigo-800 ring-indigo-200';
  if (variant === 'number') bg = 'bg-teal-100 text-teal-800 ring-teal-200';
  if (variant === 'polarity') bg = 'bg-green-100 text-green-800 ring-green-200';
  if (variant === 'question') bg = 'bg-yellow-100 text-yellow-800 ring-yellow-200';
  if (variant === 'recommendation') bg = 'bg-pink-100 text-pink-800 ring-pink-200';
  return (
    <div className={`${badgeBase} ${bg}`} aria-label={label}>
      {label}
    </div>
  );
};
                ...
                <div key={idx} className="space-y-4">
                  <div className="bg-gradient-to-br from-pink-100 to-white border-l-4 border-pink-400 p-4 rounded-xl shadow-lg text-pink-800 text-base font-semibold">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                      ...
                      <div className="flex flex-wrap ml-0 sm:ml-4">
                        ...
                        {msg.recommendation &&
                          classificationBadge(msg.recommendation, 'recommendation')}
                      </div>
                    </div>
                  </div>
